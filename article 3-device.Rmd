...
We found significant disagreement in the measurements of deep and REM sleep. The Oura Ring measures less deep sleep than the Withings Sleep, and measures more REM sleep than the Withings. Compared to the normal ranges reported by Merica, the Oura's measurment of REM sleep is unreasonably high, with most nights falling above 2 standard deviations. Similarly, the Oura's measurement of deep sleep is unreasonably low, with almost all measurements at least 1 standard deviation below normal. The Withings measurements are much more reasonable, producing values that are most often within 1 standard deviation of the normal.


## Data Access

An important question for users wanting to analyze their own data: how easy is it to download my data? For example, in preparing this article, it would have been very helpful to have been able to grab my data directly from their servers via R using each device's API. The APIs, however, require registration of an app (it assumes you are developing), and a callback URL to a server so that it can asynchronously return data to you. Using the API, therefore, requires a higher level of development than is practical for a single user. The alternative involves going to each device's web portal, and downloading the data via options there. 

In the case of the Withings Sleep, a few clicks requests all your data in the form of csv files, and an email quickly follows with a link to download all your data. The data that arrives is detailed -- one of the csv files allows you to retrieve minute-to-minute estimates of heart rate, respiratory rate, and sleep state, while another csv allows you a simpler table of time spent in each sleep state (like that used in this article).

In the case of the Oura Ring, a few clicks downloads all your data in the form of a JSON file. Oura reports minute-by-minute activity levels in metabolic-equivalent minutes, and heart rates and hypnogram categories computed for 5-minute epochs. It's 


## Return Policies

Both Oura Ring's [return policy](https://ouraring.com/returns/) and Withing's [return policy](https://support.withings.com/hc/en-us/articles/217687768-Withings-Webstore-Europe-I-am-not-satisfied-with-my-order-What-should-I-do-) state that the products can be returned for any reason within 30 days of receipt for a full refund.
